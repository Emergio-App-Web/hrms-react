...
import { getBusinessUnit, addBusinessUnit, getUser, editBusinessUnit, removeBusinessUnit } from "@/services/admin/apiMethods";
import { useEffect, useState } from "react";
import { useForm, SubmitHandler } from "react-hook-form";
import { toast } from "react-toastify";


export default function AdminBusinessUnit() {
    interface BusinessUnitTypes {
        title: string;
        description: string;
        businessunit_head: {
            name: string;
    ...
    const [businessunitContent, setBusinessUnitContent] = useState<any[]>([]);
    ...
    const { register, handleSubmit, setValue, reset } = useForm<BusinessUnitData>({
        defaultValues: {
            title: "",
            description: "",
            businessunit_head: 0,
    ...
    const editDept = (deptKey: number) => {
        ...
        if (currentDept) {
            ...
            console.error("BusinessUnit not found :", deptKey);
        }
    };
    ...
    return (
        <>
            <div className=" flex items-center gap-3 ml-11">
                <h1 className="font-montserrat font-semibold text-2xl ">BusinessUnit</h1>
                ...
                <CardContent className="p-5 lg:flex justify-end">
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-[200px_1fr] gap-4 items-center">
                            <select
                                style={{ color: "grey" }}
                                {...register("businessunit_head")}
                                onChange={(e) => {
                                    setValue("businessunit_head", parseInt(e.target.value));
                                    e.target.style.color = e.target.value === "0" ? "grey" : "black";
                                }}
                                className="bg-[#F0F0F0] p-2 rounded-xl"
                            >
                                <option value={0}>BusinessUnit Head</option>
